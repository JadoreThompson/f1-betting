...
from .config import BPATH, DPATH
from .utils import prepare_train_dataset, sma

TARGET_LABEL = "position_text"
MODEL_TYPE = ydf.GradientBoostedTreesLearner
LEARNER = ydf.GradientBoostedTreesLearner(label=TARGET_LABEL)
# [(last_n_races, sma, success_rate)]
...
def save_model(model, name: str) -> None:
    ...
    model.save(os.path.join(fpath, name))


def train(
    df: pd.DataFrame,
    year: int = 2024,
) -> Tuple[np.ndarray, MODEL_TYPE, pd.DataFrame]:
...
def process_results(results: Results, df: pd.DataFrame) -> MODEL_TYPE | None:
    """Process the results of the model training and save the best model."""
    results_df = pd.DataFrame(results, columns=["last_n_races", "sma", "success_rate"])
    ...
    if (max_result := max(res[2] for res in results)) > 0.8:
        ...
        return run_train(df, model_name="model_1")
...
def run_train_new() -> None:
    ...
    for i in range(races):
        ...
        for j in range(races):
            ...
            if not saved:
                df.to_csv(os.path.join(BPATH, "structure.csv"), index=False)
                saved = True

            if len(df[TARGET_LABEL].unique()) < 2:
                break

            _, model, test_df = train(df, 2020)
    ...
    print("\n\nOutcomes", end=f"\n{"*" * 20}\n")
    for o in outcomes:
        print(f"Last n Races: {o[0]}, SMA: {o[1]}, Success Rate: {o[2]:.2%}")
