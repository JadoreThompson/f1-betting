...
from config import F1_API_BASE_URL, BASE_PATH
...
@route.get("/upcoming-race")
async def get_upcoming_race(db_sess: AsyncSession = Depends(depends_db_session)):
    return RedirectResponse(url=f"{F1_API_BASE_URL}/{datetime.now().year}")
